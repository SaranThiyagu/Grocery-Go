            ...
            <div className="flex items-center gap-8">
              <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => router.push('/admin')}>
                <div className="w-[26px] h-[26px] rounded-[7px] bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-sm shadow-indigo-500/25">
                  <span className="text-white font-bold text-[10px] tracking-tight leading-none">GG</span>
                </div>
                <span className="text-[15px] font-semibold text-white tracking-[-0.01em]">Grocery-Go</span>
              </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.href}
                onClick={() => router.push(item.href)}
                className={`flex flex-col items-center justify-center gap-0.5 flex-1 h-full rounded-lg transition-colors cursor-pointer ${
                  active ? 'text-indigo-600' : 'text-slate-400 active:text-slate-600'
                }`}
              >
                <item.icon className={`h-[18px] w-[18px] ${active ? 'stroke-[2.5]' : ''}`} />
                <span className={`text-[10px] leading-tight ${active ? 'font-semibold' : 'font-medium'}`}>
                  {item.label}
                </span>
                {active && (
                  <div className="absolute top-0 w-8 h-0.5 rounded-full bg-indigo-600" />
                )}
              </button>
            );
          })}
